function [BestFeatures] = findFeatures(PSD_,labels_,trial_labels, TrialNb)
%findFeatures
...
PSD = PSD_(trial_labels==TrialNb,:,:);
labels = labels_(trial_labels==TrialNb);
...
ylabel('Channels')
xlabel('Frequencies')
title('Fisher Score for all couple channels/frequencies for trial ' + num2str(TrialNb));
...
for i=1:10
    [~, I_col] = max(max(FisherScore)); %I_col == column == best channel
    [~, WI] = max(FisherScore);
