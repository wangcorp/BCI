...
[data_nofilter_1] = s1(:,1:16);
[data_nofilter_2] = s2(:,1:16);
[data_nofilter_3] = s3(:,1:16);

%% Power

PW = PowerWelch(data_lap_2); %Insert data of interest
PlotWelch(PW)
...
[~, f_nofilter_1, t_nofilter_1, PSD_nofilter_1] = Spectrogram16(data_nofilter_1);
[~, f_nofilter_2, t_nofilter_2, PSD_nofilter_2] = Spectrogram16(data_nofilter_2);
[~, f_nofilter_3, t_nofilter_3, PSD_nofilter_3] = Spectrogram16(data_nofilter_3);
...
[PSD_nofilter, event_nofilter] = ConcatenatePSD(PSD_nofilter_1, PSD_nofilter_2, PSD_nofilter_3, h1, h2, h3);

labels_runs = [ones(1, length(PSD_car_1)), 2*ones(1, length(PSD_car_2)), 3*ones(1, length(PSD_car_3))]';
labels_car = MakeLabels(PSD_car, event_car);
labels_lap = MakeLabels(PSD_lap, event_lap);

%% Save PSD and other useful variables
save('PSD.mat', 'PSD_car', 'PSD_lap', 'PSD_nofilter', 'labels_car', 'labels_lap', 'labels_runs', 'event_car', 'event_car');