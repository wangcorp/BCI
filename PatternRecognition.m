...
sel_freq = [6,6,6]; %to fill according to the selected best features (frequencies)
sel_chan = [12,11,7]; %to fill according to the selected best features (channels)
        ...
        for windowID = 2:length(PSD_feat_lap_online)
            ...
            if trial_check == true
                ...
                if dt >= y_up
                    ...
                    decision_time(trial_counter)=trial_time;%-event_lap(event_trial(current_trial),2);
                    trial_check = false;
                elseif dt <= y_down
...
%time estimation
decision_time_mean = mean(decision_time(decision_time~=0));
